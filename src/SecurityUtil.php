...
class SecurityUtil
{
     ...
     * @return string|null
     */
    public static function createPasswordHash(string $password, string|int $algo = PASSWORD_BCRYPT, array $options = []): ?string
    {
        if (empty($options))
        {
            $options = [
    ...
    public static function createRandomToken(int $length = 12, ?string $prefix = null, ?string $customCharacters = null): string
    {
        ...
        if (empty($customCharacters) === false)
        {
        ...
        if ($prefix !== null)
        {
            ...
            if ($length < 0)
            {
                throw new \InvalidArgumentException('Prefix length is too long.');
            }
        }
